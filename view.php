...
<h1 class="title">Galeria Imagens</h1>
...
foreach ($arquivosImagens as $arquivo) {
    ...
    $caminhoImagem = $diretorioImagens . $arquivo;

    // Abrir arquivo de descrição
    $arquivoDescricao = $diretorioImagens . $nomeImagem . ".txt";
    if (file_exists($arquivoDescricao)) {
        $descricaoImagem = file_get_contents($arquivoDescricao);
    } else {
        $descricaoImagem = "<hr>" . "<p>" . "Descrição:" . "</p>";
    }

    // Exibir a imagem e sua descrição
    echo "<div class='item-postado'>";
    echo "<img src='" . $caminhoImagem . "' alt='' width='200px" . $tamanhoImagem . "'>";
    echo "<p>" . $descricaoImagem . "</p>";
    echo "</div>";
}
